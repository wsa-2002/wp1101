...
const Board = ({ boardSize, mineNum, backToHome }) => {
  ...
  const freshBoard = () => {
    ...
    let data = createBoard(boardSize, mineNum);
    setBoard(data.board);
        ...
        for (let j = 0; j < board[i].length; j++) {
          if (i === x && j === y) {
            ...
            subcol.push({
              ...
              revealed: board[i][j].revealed, // To store if the cell is revealed.
  ...
  const revealCell = (x, y) => {
    ...
    if (board[x][y].flagged === true) return;
    if (win === true) return;
    if (gameOver === true) return;
    let data = revealed(board, x, y, nonMineCount);
